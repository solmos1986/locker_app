...
class DoorRepository {
  ...
  Future<List<DoorTotalModel>> readDoorAvailable(int doorSizeId) async {
    final db = await LockeAppDatabase.instance.database;
    String query =
        "SELECT door.name, door.door_id, door_size.name as name_size FROM door INNER JOIN door_size on door.door_size_id = door_size.door_size_id WHERE door.state=1 and door_size.door_size_id=${doorSizeId.toString()};";
